...
const useStyles = makeStyles((theme) => ({
    ...
    image: {
        backgroundImage: 'url(https://images.unsplash.com/photo-1463173904305-ba479d2123b7?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=2026&q=80)',
...
const Login = (props) => {
    ...
    const login = async (e) => {
        ...
        if (response && response.status === 200) {
            ...
            props.setApiKey(response.data.apiKey);
    ...
    return (
        <Grid container component="main" className={classes.root}>
            ...
            <Dialog open={openError} TransitionComponent={Transition} keepMounted onClose={() => setOpenError(false)} aria-labelledby="alert-dialog-slide-title" aria-describedby="alert-dialog-slide-description">
                <DialogTitle id="alert-dialog-slide-title">{"Wrong username or password"}</DialogTitle>
                <DialogContent>
                    <DialogContentText id="alert-dialog-slide-description">
                        The credentials you are using were not found.
